...
DEVICE_TYPE_AIR_PURIFIERS = "Air Purifiers"
...
class Mill:
    ...
    async def _update_device(self, device_data: dict[str, Any], room_data: dict[str, Any] | None = None) -> None:
        ...
        elif device_type == DEVICE_TYPE_AIR_PURIFIERS:
            self.devices[device_id] = AirPurifier.init_from_response(device_data)
        else:
...
@dataclass()
class AirPurifier(Sensor):
    """Representation of an air purifier with environmental sensors."""

    air_quality_index: float | None = None
    control_signal: float | None = None
    fan_level_auto: float | None = None
    temperature_level: float | None = None
    humidity_level: float | None = None
    eco2_level: float | None = None
    tvoc_level: float | None = None
    particles10_level: float | None = None
    particles25_level: float | None = None
    particles100_level: float | None = None

    def __post_init__(self) -> None:
        """Initialize air purifier from device data."""
        super().__post_init__()
        if not self.data:
            return

        last_metrics = self.data.get("lastMetrics", {})
        self.air_quality_index = last_metrics.get("airQualityIndex")
        self.control_signal = last_metrics.get("controlSignal")
        self.fan_level_auto = last_metrics.get("setFanLevelInAuto")
        self.temperature_level = last_metrics.get("temperatureLevel")
        self.humidity_level = last_metrics.get("humidityLevel")
        self.eco2_level = last_metrics.get("eco2Level")
        self.tvoc_level = last_metrics.get("tvocLevel")
        self.particles10_level = last_metrics.get("particles10Level")
        self.particles25_level = last_metrics.get("particles25Level")
        self.particles100_level = last_metrics.get("particles100Level")

    @property
    def device_type(self) -> str:
        """Return device type."""
        return DEVICE_TYPE_AIR_PURIFIERS
