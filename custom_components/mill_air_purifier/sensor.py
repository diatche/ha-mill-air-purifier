...
HEATER_SENSOR_TYPES: tuple[SensorEntityDescription, ...] = (
    ...
    SensorEntityDescription(
        key="current_room_mode",
        translation_key="current_room_mode",
    ),
    SensorEntityDescription(
        key="heat_status",
    ...
    SensorEntityDescription(
        key="room_avg_temp",
        translation_key="average_room_temperature",
        device_class=SensorDeviceClass.TEMPERATURE,
        native_unit_of_measurement=UnitOfTemperature.CELSIUS,
        state_class=SensorStateClass.MEASUREMENT,
    ),
...
AIR_PURIFIER_SENSOR_TYPES: tuple[SensorEntityDescription, ...] = (
    ...
    SensorEntityDescription(
        key="eco2_level",
        name="Estimated CO2 level",
        translation_key="estimated_co2_level",
        state_class=SensorStateClass.MEASUREMENT,
        entity_category=EntityCategory.DIAGNOSTIC,
    ),
    SensorEntityDescription(
    ...
    SensorEntityDescription(
        key="fan_level_auto",
        name="Fan level auto",
        translation_key="fan_level_auto",
        state_class=SensorStateClass.MEASUREMENT,
    ),
)
...
async def async_setup_entry(
    ...
    entities = [
        ...
        for entity_description in (
            SOCKET_SENSOR_TYPES
            if isinstance(mill_device, pymill.Socket)
            else HEATER_SENSOR_TYPES
            if isinstance(mill_device, pymill.Heater)
            else AIR_PURIFIER_SENSOR_TYPES
            if isinstance(mill_device, pymill.AirPurifier)
            else SENSOR_TYPES
        )
    ]
...
class MillSensor(MillBaseEntity, SensorEntity):
    ...
    def __init__(
        ...
        mill_device: pymill.Socket | pymill.Heater,
    ) -> None:
